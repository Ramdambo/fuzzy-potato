...
def QuatToRot(q0, q1, q2, q3):
    R = [[2 * q0 * q0 - 1 + 2 * q1 * q1, 2 * q1 * q2 - 2 * q0 * q3, 2 * q1 * q3 + 2 * q0 * q2],
         [2 * q1 * q2 + 2 * q0 * q3, 2 * q0 * q0 - 1 +
             2 * q2 * q2, 2 * q2 * q3 - 2 * q0 * q1],
...
def plotcirc3d(n1, n2, n3, ur1, ur2, ur3, radius, ax):
    ...
    for t in np.arange(0, 2 * math.pi, 0.02):
        pcircle[i, :] = (radius * math.cos(t) * u) + radius * \
            math.sin(t) * np.cross(n, u) + Ur
        i = i + 1
...
def circle_intpl(points):
    ...
    for i in range(0, nr):
        Prot[i] = np.subtract(np.transpose(A[i][:]),
                              np.multiply(np.matmul(A[i][:], n), n))
    ...
    Ur = np.matmul(np.transpose(R), np.array(
        [xc, yc, 0], dtype=object)) + meanp
    Prot1 = np.transpose(Prot1) + mtlb.repmat(meanp, nr, 1)
    # fig = plt.figure()
    # ax = fig.add_subplot(111, projection='3d')
    # ax.scatter(Prot1[:, 0], Prot1[:, 1], Prot1[:, 2])
    points = np.array(points)
    up = np.cross(np.subtract(points[0, :], [Ur[0][0], Ur[1][0], Ur[2][0]]), np.subtract(
        points[3, :], [Ur[0][0], Ur[1][0], Ur[2][0]]))
